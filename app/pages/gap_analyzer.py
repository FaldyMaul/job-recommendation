...
async def run_all_agents():
    ...
    plan = await generate_learning_plan(job, cv_summary, gaps.get("gaps", []), requirements.get("requirements", []))
...
with st.spinner("🔍 Analyzing your profile against the job requirements..."):
    try:
        ...
        gaps, requirements, learning_plan = {"gaps": []}, {"requirements": []}, {"plan": []}
...
for week in learning_plan.get("plan", []):
    st.markdown(f"### Week {week['week']}: {week['focus_area']}")
    st.markdown(f"**Objectives:** {week['objectives']}")
    st.markdown(f"**Activities:**")
    for activity in week.get("activities", []):
        for gap in priority_gaps:
            if gap.lower() in activity.lower():
                activity = f"⭐ {activity}"
                break
    ...
    st.markdown(f"**Rationale:** {week['rationale']}")
    st.divider()
